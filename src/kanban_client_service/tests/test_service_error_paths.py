...
def test_authorization_header_is_used() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.get_boards", autospec=True) as mock_get:
        mock_get.return_value = [KanbanBoard(id="b1", name="N", description=None, closed=False, url=None)]
...
def test_users_me_auth_error_returns_401() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.get_current_user", autospec=True) as mock_get:
        mock_get.side_effect = KanbanAuthenticationError()
...
def test_boards_api_error_returns_400() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.get_boards", autospec=True) as mock_get:
        mock_get.side_effect = KanbanAPIError("bad", 400)
        resp = client.get("/boards", headers={"Authorization": "Bearer T"})
...
def test_board_not_found_returns_404() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.get_board", autospec=True) as mock_get:
        mock_get.side_effect = KanbanNotFoundError("missing")
...
def test_delete_board_api_error_returns_400() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.delete_board", autospec=True) as mock_del:
        mock_del.side_effect = KanbanAPIError("bad", 400)
...
def test_lists_not_found_returns_404() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.get_lists", autospec=True) as mock_get:
        mock_get.side_effect = KanbanNotFoundError("no lists")
...
def test_create_list_api_error_returns_400() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.create_list", autospec=True) as mock_create:
        mock_create.side_effect = KanbanAPIError("bad", 400)
...
def test_update_list_api_error_returns_400() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.update_list", autospec=True) as mock_upd:
        mock_upd.side_effect = KanbanAPIError("bad", 400)
...
def test_get_cards_api_error_returns_400() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.get_cards", autospec=True) as mock_get:
        mock_get.side_effect = KanbanAPIError("bad", 400)
...
def test_get_card_not_found_returns_404() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.get_card", autospec=True) as mock_get:
        mock_get.side_effect = KanbanNotFoundError("missing")
...
def test_create_card_api_error_returns_400() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.create_card", autospec=True) as mock_create:
        mock_create.side_effect = KanbanAPIError("bad", 400)
        resp = client.post(
            "/lists/ly/cards",
...
def test_update_card_api_error_returns_400() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.update_card", autospec=True) as mock_update:
        mock_update.side_effect = KanbanAPIError("bad", 400)
        resp = client.put(
            "/cards/cx",
            params={"name": "C"},
            headers={"Authorization": "Bearer T"},
...
def test_delete_card_api_error_returns_400() -> None:
    ...
    with patch("kanban_client_service.main.TrelloClientImpl.delete_card", autospec=True) as mock_del:
        mock_del.side_effect = KanbanAPIError("bad", 400)
